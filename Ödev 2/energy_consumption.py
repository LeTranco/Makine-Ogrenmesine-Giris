# gerekli kütüphnelerin eklenmesi
import pandas as pd 
import numpy as np
...
# 'time' sütünunun sayısallaştırıp oluşan sayısal zaman verisinin okunulabilir tarih formatına çevrilmesi
...
# 'summary' sütununun sayısllaştırılması
le = LabelEncoder() 
df['summary_encoded'] = le.fit_transform(df['summary'].astype(str)) 

# her saat, sıcaklık, nem ve rüzgar hızı için ortalama enerji tüketimini hesaplıyan bir pivot tablo oluşturulması
...
# sırayla modellerin eğitilmesi, test verilerinin üzernde tahmin yapılması ve doğruluk oranlarının atanması
models = {
