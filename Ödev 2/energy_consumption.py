...
weather_cols = ['temperature', 'humidity', 'visibility', 'windSpeed', 'pressure', 'cloudCover', 'windBearing', 'dewPoint', 'precipProbability']
...
sns.lineplot(ax=axes[0, 0], x=pivot_hour.index, y=pivot_hour['hourAvg'], marker='o', color='blue') 
axes[0, 0].set_title('Average Hourly Consumption')
axes[0, 0].set_ylabel('Avarage kW') 
sns.scatterplot(ax=axes[0, 1], x=pivot_temp.index, y=pivot_temp['temperatureAvg'], color='orange', alpha=0.5) 
axes[0, 1].set_title('Average Consumption Based on Temperature') 
sns.lineplot(ax=axes[1, 0], x=pivot_humid.index, y=pivot_humid['humidityAvg'], color='green') 
axes[1, 0].set_title('Average Consumption Based on Humidity') 
sns.lineplot(ax=axes[1, 1], x=pivot_wind.index, y=pivot_wind['windSpeedAvg'], color='red') 
...
fi_df = pd.DataFrame({'Feature': features, 'Importance': rf_importance}).sort_values(by='Importance', ascending=False) 
